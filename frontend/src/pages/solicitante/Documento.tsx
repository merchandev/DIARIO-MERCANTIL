...
export default function Documento(){
  ...
  const uploadPdfAnalysis = async(e:any)=>{
    ...
    try {
      ...
      console.log('Cálculo de precios:', { folios, priceUsd, pricePerFolio, bcvRate, subtotalBs, ivaBs, totalBs })
      
      // Paso 3: Subir el archivo al backend
      const formData = new FormData()
      formData.append('file', file)
      
      const token = localStorage.getItem('token') || sessionStorage.getItem('token')
      if (!token) {
        setAlertDialog({isOpen:true, title:'Sesión expirada', message:'Su sesión ha expirado. Por favor inicie sesión nuevamente.', variant:'error'})
      ...
      console.log('Enviando PDF al servidor con token de autenticación...')
      ...
      <div className="card p-6 space-y-4 shadow-md step-anim">
        <div className="flex items-center gap-3 pb-3 border-b">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg ${
            step >= 2 ? 'bg-brand-100 text-brand-700' : 'bg-slate-200 text-slate-500'
          }`}>2</div>
          <div>
