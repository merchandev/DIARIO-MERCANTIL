...
export default function Sidebar({onPublishClick, onCollapseChange}:{onPublishClick?:()=>void; onCollapseChange?:(collapsed:boolean)=>void}){
  ...
  const [user, setUser] = useState<{id?:number; name?:string; role?:string; document?:string}|null>(storedName||storedRole||storedDoc ? { name: storedName, role: storedRole, document: storedDoc } : null)
